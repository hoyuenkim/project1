  ...
  try {
    ...
    await Promise.all(
      await req.body.ProductIds.map((v) => {
        return db.Discount.create({ ProductId: v.ProductId, rate: req.body.rate });
      }),
    );
    const Products = await db.Product.findAll({
      where: { ShopId: req.body.ShopId },
...
router.post("/category/add", async (req, res, next) => {
  try {
    const category = await db.Category.create({ name: req.body.name, ShopId: req.body.ShopId });
...
router.post("/category/update", async (req, res, next) => {
  ...
  try {
    await db.Category.update({ name: req.body.name }, { where: { id: req.body.id } });
